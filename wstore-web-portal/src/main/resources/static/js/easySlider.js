...
(function ($, window, document, undefined) {
    ...
    $.extend(Plugin.prototype, {
        ...
        events: {
            clickRight: function () {
                ...
                $(this.settings.controlsClass + " " + "li:last-child").click(
                    function () {
                        if (_this.slideParameters.getCurrentSlideNumber.call(_this) === _this.slideParameters
                            .getNumberOfSlides.call(_this)) {
                            //go to first slide, when slideshow has reached max distance
                            $(_this.settings.slidesClass).animate({
                                right: "0%"
                            }, _this.settings.slideSpeed);
                            ...
                            _this.slideParameters.setCurrentSlideNumber.call(_this, _this.slideParameters
                                .getCurrentSlideNumber.call(_this) + 1);
                            _this.paginate(_this.slideParameters.getCurrentSlideNumber.call(_this));
                        }
                    });
            },
                ...
                $(this.settings.controlsClass + " " + "li:first-child").click(
                    function () {
                        if (_this.slideParameters.getCurrentSlideNumber.call(_this) === 1) {
                            //go to first slide, when slideshow has reached max distance
                            $(_this.settings.slidesClass).animate({
                                right: (_this.slideParameters.getMaxSlidePercentage.call(
                                    _this) - 100).toString() + "%"
                            }, _this.settings.slideSpeed);
                            _this.slideParameters.setCurrentSlideNumber.call(_this, _this.slideParameters
                                .getNumberOfSlides.call(_this));
            ...
            clickPage: function () {
                ...
                $(this.settings.paginationClass + " " + "li").click(
                    function () {
                        ...
                        $(_this.settings.slidesClass).animate({
                            right: ((currentSlideNumber - 1) * 100).toString() + "%"
                        }, 500);
        ...
        paginate: function (currentSlideNumber) {
            ...
            for (i = 1; i <= total; i++) {
                $(this.settings.paginationClass + " " + "li:nth-child" + "(" + i.toString() + ")").removeClass(
                    "active");
            }
            $(this.settings.paginationClass + " " + "li:nth-child" + "(" + currentSlideNumber.toString() +
                ")").addClass("active");
        ...
        positionPagination: function () {
            ...
            var marginLeft = -(numberOfSlides * (this.convertStringToInteger(this.settings.paginationDiameter)) +
                (numberOfSlides - 1) * (this.convertStringToInteger(this.settings.paginationSpacing))
            ) / 2;

            $(this.settings.paginationClass).css("margin-left", marginLeft);
        },
        positionControls: function () {
            var heightDiff = (this.slideParameters.getSliderHeight.call(this) - this.slideParameters.getSlideHeight
                .call(this)) / 2;
            var halfHeightControl = $(this.settings.controlsClass + " " + "li").css("margin-top");

            $(this.settings.controlsClass + " " + "li").css("margin-top", ((this.convertStringToInteger(
                halfHeightControl)) - heightDiff) + "px");
        },
        slideParameters: {
